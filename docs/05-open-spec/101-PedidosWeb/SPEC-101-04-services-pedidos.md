...
- **Copia completa de cabecera (CC PQ #15, D1-29/D1-30):** `ComprobanteCopiaService::copiarBorrador` incluye `id_de` y `leyenda_1`…`leyenda_5` del origen en `resultado.borrador.cabecera`. Las leyendas pasan por el recorte canónico a 60; nulos/vacíos se conservan. No cambia la política `ActualizarPrecioCopia`.
...
- **Tope de leyendas (CC PQ #13):** al grabar pedido o presupuesto —alta, edición o conversión que persiste cabecera— aplicar el helper canónico `recortarLeyendaCabecera` (`mb_substr($texto, 0, 60)`) a `leyenda_1`…`leyenda_5`. Aceptar textos más largos y recortarlos; no responder 4xx por longitud. El valor sincronizado al maestro cliente ya debe estar recortado. `CabeceraInicialService` también recorta valores legados del maestro. OpenAPI documenta `maxLength: 60` como longitud persistida.
...
- [x] CC PQ #13: grabación e hidratación recortan leyendas a 60 sin rechazo por longitud
- [x] **CA-CC15-C01:** copia conserva `id_de` del origen
- [x] **CA-CC15-C02:** copia conserva `leyenda_1`…`leyenda_5`, recortadas a 60 si exceden
- [x] **CA-CC15-C03:** copia admite dirección y leyendas nulas/vacías sin error
- [x] **CA-CC15-C04:** precios y `ActualizarPrecioCopia` mantienen su comportamiento
...
## Historial CC PQ #13 y #15 — Parte I 12/09/2026

- CC PQ #13: unificación `SPEC-101-04-services-pedidos-update`; recorte Unicode a 60 al grabar/hidratar, sync al maestro ya acotado y contrato OpenAPI.
- CC PQ #15: unificación posterior `SPEC-101-04-services-pedidos-update-01`; copia de `id_de` y leyendas 1–5, con D1-29/D1-30 y CA-CC15-C01…C04 incorporados.
