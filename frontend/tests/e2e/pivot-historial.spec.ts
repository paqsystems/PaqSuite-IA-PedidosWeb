...
async function mockPivotHistorialApi(page: import('@playwright/test').Page) {
...
test('historial alterna grilla a pivot', async ({ page }) => {
  await mockPivotHistorialApi(page);
  ...
  await expect(page.getByTestId('historialVentas.gridView')).toBeVisible();
