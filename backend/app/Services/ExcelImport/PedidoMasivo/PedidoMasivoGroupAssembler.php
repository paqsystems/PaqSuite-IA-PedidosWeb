...
final class PedidoMasivoGroupAssembler
{
    ...
    public function assemble(array $validItems, User $user): array
    {
        ...
        foreach ($validItems as $item) {
            ...
            $groupKey = strtolower($codCliente).'|'.$codVended.'|'.$nivel;
    ...
    private function buildCabeceraFromRow(array $row, array $cabeceraBase): array
    {
        return [
            ...
            'cod_condvta' => (int) ($row['cod_condvta'] ?? $cabeceraBase['cod_condvta'] ?? 0),
            'cod_transpor' => $row['cod_transpor'] ?? $cabeceraBase['cod_transpor'] ?? null,
            'id_de' => (int) ($row['id_de'] ?? $cabeceraBase['id_de'] ?? 0),
            'nivel' => PedidoExcelImportCabeceraSignature::resolveNivelForGroupKey($row['nivel'] ?? $cabeceraBase['nivel'] ?? 0),
            'lista_precios' => (int) ($row['cod_lista'] ?? $cabeceraBase['lista_precios'] ?? 0),
            ...
            'expreso' => $row['expreso'] ?? $cabeceraBase['expreso'] ?? null,
            'expreso_dire' => $row['expreso_dire'] ?? $cabeceraBase['expreso_dire'] ?? null,
            'fecha_entrega' => $row['fecha_entrega'] ?? $cabeceraBase['fecha_entrega'] ?? null,
            'observaciones' => (string) ($row['observaciones'] ?? $cabeceraBase['observaciones'] ?? ''),
            'cod_perfil' => (string) ($row['cod_perfil'] ?? $cabeceraBase['cod_perfil'] ?? ''),
